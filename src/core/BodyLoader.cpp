                                   ...
                                   const MotorRegistry& motors)
{
    ...
    // Resolve mesh paths relative to the body JSON's own directory.
    // Paths starting with .. are relative to body_dir.
    // Paths starting with a plain name (e.g. "assets/...") are cwd-relative
    // and left alone — just normalised to forward slashes for Windows.
    fs::path body_dir = fs::path(json_path).parent_path();
    auto resolve_mesh = [&](const std::string &raw) -> std::string {
        if (raw.empty()) return "";
        fs::path p(raw);
        if (p.is_absolute() || p.begin()->string() != "..")
            return p.lexically_normal().generic_string();
        return (body_dir / p).lexically_normal().generic_string();
    };
 
    def.mesh_path           = resolve_mesh(j.value("mesh", ""));
    def.collision_mesh_path = resolve_mesh(j.value("collision_mesh", ""));
